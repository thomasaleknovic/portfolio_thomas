...
function Contact () {
  ...
  const sendEmail = (e) => {
    ...
    if (email && name ){
          emailjs.sendForm('service_31qmlfw', 'template_kas4clh', form.current, 'fa43rj5b0dL1JZ-lC')
                .then((result) => {
  ...
  function validateEmail(value) {
    ...
    } else if (!/[a-z0-9!#$%&'*+/=?^_`{|}~-]+(?:\.[a-z0-9!#$%&'*+/=?^_`{|}~-]+)*@(?:[a-z0-9](?:[a-z0-9-]*[a-z0-9])?\.)+[a-z0-9](?:[a-z0-9-]*[a-z0-9])?/gi
    .test(value)) {
      ...
      <div id='contato' className='flex flex-col items-center pb-[40px] pt-[100px]'>
          ...
          <div className='bg-mainPurple w-5/6 md:w-2/4 rounded-md p-10'>
            <form ref={form} onSubmit={sendEmail} className="flex flex-col items-center text-mainBlack">
                ...
                <input onBlur={handleBlur} type="text" name="user_name" className={`md:w-[400px] text-center mb-2 mt-3 p-3 rounded-md ${nameError?'shadow-[0px_0px_10px] shadow-red-700' : ''}`}/>
                <p className='font-bold text-red-600 mb-6'>{nameError}</p>

                <label className='text-white font-bold'>Email</label>
                <input onBlur={handleBlur} type="email" name="user_email" className={`md:w-[400px] text-center mb-2 mt-3 p-3 rounded-md ${emailError?'shadow-[0px_0px_10px] shadow-red-700' : ''}`}/>
